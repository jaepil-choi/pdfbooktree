"""experiment 093: font size와 normalized position을 joint style로 묶는다.
...
실행:
    uv run python experiments/093_joint_position_font_style_level.py
"""
...
EXPERIMENT_ID = "093_joint_position_font_style_level"
...
def evaluate_truth(
    ...
    return {
        ...
        "label_quality": "near-answer weak reference; human review required",
...
def record_experiment(summary: dict[str, Any]) -> None:
    ...
    entry = {
        ...
        "method": "모든 visual line의 top-left를 page-relative 좌표로 바꾸고 문서 median line-height를 단위로 환산했다. body font median과 font-size가 상대적으로 다른 line만 대상으로, font ratio와 x/y가 각각 dimensionless tolerance 안에 드는 joint style을 만들었다. 2개 이상 page에 반복되는 style만 남기고 style median font-size stack으로 level을 부여했다.",
        "summary": compact,
...
def main() -> None:
    ...
    best = max(
        results,
        key=lambda result: (
            result["truth_evaluation"]["level_correct"],
            result["truth_evaluation"]["matched"],
    ...
    finding = (
        f"body tier={body_tier}, body font median={body_font_size:.4f}, visual lines={len(lines)}, truth={len(truth)}. "
        f"joint style 비교={comparison}. near-answer level을 가장 많이 맞힌 config={best_config}, "
        f"recall={best['truth_evaluation']['recall']}, level_accuracy={best['truth_evaluation']['level_accuracy']}, "
        f"level_correct={best['truth_evaluation']['level_correct']}/{len(truth)}이다."
    )
    summary = {
