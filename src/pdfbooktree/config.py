...
class ProcessingConfig:
    ...
    # True면 런타임에서 LLM range review와 clustered item extraction을 실제 호출한다.
    use_llm: bool = False
    ...
    # 최신 clustered staged extractor가 runtime item 추출을 소유한다.
...
# clustered staged extractor 1단계 prompt. experiment 029/030에서 채택한
# "결정론 클러스터 + LLM 클러스터 순서/병합" 전략이다.
DEFAULT_STAGED_CLUSTER_SYSTEM_PROMPT = (
    "너는 책 목차의 계층을 정하는 도구다. 입력은 목차 줄들을 이미 묶어 놓은 "
    "'클러스터' 목록이다. 각 클러스터에는 글꼴(font_type), 굵기(is_bold), "
    "평균 글씨 높이(mean_height), 들여쓰기 컬럼(indent_col, 작을수록 왼쪽), "
    "줄 수(count), 페이지번호 비율(page_number_frac), 대표 제목 예시(examples)가 있다.\n"
    "네 일은 '각 클러스터의 계층 level을 정하는 것'뿐이다(1=최상위). 규칙:\n"
    "- 줄을 다시 묶거나 나누지 마라. 오직 클러스터마다 level 정수 하나만 부여한다.\n"
    "- 큰 글씨/굵게/왼쪽일수록, 그리고 예시 제목이 장/부(chapter/part)처럼 상위 단위면 "
    "상위 level이다. 작은 글씨/들여쓰기/세부 항목 예시는 하위 level이다.\n"
    "- 병합: 여러 클러스터가 같은 구조 단위면 같은 level을 줘라. 예시가 둘 다 "
    "'Chapter N'으로 시작하는데 들여쓰기 컬럼만 살짝 다르면 OCR 드리프트이니 같은 "
    "level로 합쳐라.\n"
    "- 분리: 글꼴 계열(font_type)이 서로 다르고 예시 제목이 서로 다른 구조 단위이면, "
    "글씨 높이가 비슷해도 별도 level로 둬라.\n"
    "- 예시 제목의 의미를 반드시 활용하라. 신호가 애매하면 제목이 최종 판단 근거다."
)


# staged extractor 2단계 page extraction prompt. level은 확정된 [Ln] marker를
# 복사만 하며 LLM이 새 level을 만들 수 없다.
DEFAULT_STAGED_EXTRACT_SYSTEM_PROMPT = (
    "너는 책 목차 페이지에서 항목을 추출하는 도구다. 각 줄 앞 [Ln]은 그 줄의 "
    "계층 level이다(이미 결정됨).\n"
    "절대 규칙: 계층/level은 네가 정하지 않는다. 각 항목의 level은 그 항목이 나온 "
    "줄의 [Ln] 숫자(n)를 그대로 복사만 한다. 한 줄을 여러 항목으로 쪼개면 모든 "
    "조각은 그 줄과 같은 level을 받는다.\n"
    "너가 하는 일은 다음뿐이다.\n"
    "- 항목 분리: 한 줄에 여러 항목이 'I'/'|'/'·'/'•' 구분자와 페이지 번호로 합쳐져 있으면 "
    "각 항목으로 분리한다. 스캔 OCR이 2단 목차를 한 줄로 합치는 경우가 여기에 해당한다.\n"
    "- 페이지 번호: printed_page는 제목 뒤(또는 '•'/'·' 뒤)의 인쇄 페이지 번호. 없으면 null.\n"
    "- 제목 글씨가 깨진 줄도 빼지 말고 반드시 항목으로 낸다. 제목 교정은 다음 단계에서 한다.\n"
    "- '목차'/'Contents' 같은 머리말 한 마디 줄은 항목으로 만들지 않는다.\n"
    "- 항목은 이 페이지에 나온 순서 그대로 반환한다."
)


DEFAULT_STAGED_CORRECTION_SYSTEM_PROMPT = (
    "너는 OCR로 깨진 목차 제목을 교정하는 도구다. 입력 항목들의 title에서 "
    "OCR 오인식/깨진 글자/붙은 띄어쓰기만 자연스럽게 고친다.\n"
    "절대 규칙:\n"
    "- 항목 개수, 순서를 절대 바꾸지 마라. 입력과 1:1로 대응하는 교정된 title만 "
    "같은 순서로 반환한다.\n"
...
class LlmStagedTocExtractionConfig:
    """클러스터 기반 staged LLM TOC item 추출 설정이다.

    experiment 029/030에서 검증한 흐름을 제품 코드로 옮긴다. 코드는
    height/indent/bold/font 클러스터를 만들고, LLM은 클러스터 순서/병합과
    level marker 기반 항목 분리, 제목 교정만 수행한다.
    """
    ...
    max_completion_tokens: int | None = None
    default_item_confidence: float = 0.8
    cluster_system_prompt: str = DEFAULT_STAGED_CLUSTER_SYSTEM_PROMPT
    extract_system_prompt: str = DEFAULT_STAGED_EXTRACT_SYSTEM_PROMPT
    correction_system_prompt: str = DEFAULT_STAGED_CORRECTION_SYSTEM_PROMPT
