...
class BookmarkTocBatchDetector:
    ...
    def _detect_pdf(
        ...
        if detection.pages:
            try:
                dataset_rows, dataset_warnings = self._build_dataset_rows(
                    pdf_path=pdf_path,
                ...
                warnings.append(
                    f"dataset row 생성 실패: {type(error).__name__}: {error}"
                )

        if detection.pages:
            self._emit(
                f"TOC/dataset 생성 완료 ({index}/{total}): {display_path} "
                f"positives={len(detection.pages)} "
                f"negatives={sum(1 for row in dataset_rows if row.label == 0)} "
